...
export const useMonthlyEntriesAction = ({
  ...
  const addMutation = useAddMonthlyEntriesMutation();
  const updateMutation = useUpdateMonthlyEntriesMutation();
  const deleteMutation = useDeleteMonthlyEntriesMutation();
  ...
  const updateEntry = (
    ...
    updateMutation.mutate(
      { entry: data, entryId, budgetId, type },
      {
        onSuccess: () => {
          onSuccess?.();
        },
